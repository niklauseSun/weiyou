...
+ (void)registerLocalNotification API_AVAILABLE(ios(10.0)) {
    ...
    UNNotificationAction *action2 = [UNNotificationAction actionWithIdentifier:actionTenMin title:@"10分钟后" options:UNNotificationActionOptionNone];
    UNNotificationAction *action3 = [UNNotificationAction actionWithIdentifier:actionFiftyMin title:@"15分钟后" options:UNNotificationActionOptionNone];
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void (^)(void))completionHandler  API_AVAILABLE(ios(10.0)){
    NSLog(@"dddd");
//  [[UIApplication sharedApplication].keyWindow.rootViewController.view makeToast:@"闹铃响了"];
    completionHandler();
...
-(void)handCommnet:(UNNotificationResponse *)response API_AVAILABLE(ios(10.0)) {
    ...
    }else if ([actionIdef isEqualToString:actionTenMin]) {
        date = [NSDate dateWithTimeIntervalSinceNow:10 * 60];
    }else if ([actionIdef isEqualToString:actionFiftyMin]) {
        date = [NSDate dateWithTimeIntervalSinceNow:15 * 60];
    }
...
- (void)handleRemot:(NSDictionary *)userInfo {
  ...
  if ([type isEqualToString:@"clock"]) {
    ...
    [self handleSendNotification:[NSString stringWithFormat:@"punchNotice-%@", idStr]];
  } else {
